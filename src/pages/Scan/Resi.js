...
export default function Resi({ navigation, route }) {
    ...
    const __deleteData = (barcode) => {
        axios.delete(APIurl + 'scan?id_member=' + user.id + '&key=' + barcode).then(res => {
            console.log(res.data);
            __getTransaction();
        })
    }
    ...
    const __filterData = (barcode) => {
        // setLoading(true);
        axios.get(APIurl + 'scan?id_member=' + route.params.id + '&key=' + barcode).then(res => {
